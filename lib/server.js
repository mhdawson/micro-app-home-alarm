...
function stopPictureTimer() {
  if (pictureTimer !== undefined) {
  ...
  mqttClient.on('message', function(topic, message) {
    ...
    if (alarmStatusTopic == topic) { 
      if (('arm' == message) || ('disarm' == message)) {
        ...
        notify.sendNotification(config, 'Alarm triggered:' + config.alarmSite);
    ...
    if (undefined != zoneMapping[topic]) { 
      ...
      eventSocket.emit('message', topic + ":" + message);
    }
  });
